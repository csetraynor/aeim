...
pp_data <-
  function(object,
    ...
    if (is.stanidm(object)) {
      return(.pp_data_idm(object, newdata = newdata, ...))
    }
...
.pp_data_idm <- function(object,
  ...
  # time-fixed predictor matrix
  tf_form <- lapply(formula, function(formula) reformulate_rhs(rhs(formula$tf_form)) )
  mf <- lapply(seq_along(tf_form), function(i) make_model_frame(tf_form[[i]], newdata[[i]], check_constant = FALSE)$mf )
  x  <- lapply(seq_along(mf) , function(i) make_x(tf_form[[i]], mf[[i]], xlevs= object$xlevs[[i]], check_constant = FALSE)$x )
...
get_model_data.stanidm <- function(object, ...) {
  validate_stanidm_object(object)
  terms <- terms(object)
  row_nms <- lapply(model.frame(object), function(m) row.names(m))
  lapply(seq_along(object$data), function(i) {
    get_all_vars(terms[[i]], object$data[[i]])[row_nms[[i]], , drop = FALSE]
    })
}
